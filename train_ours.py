...
# from Modules import Unet as model_train
...
def parse_args():
    ...
    # model
    parser.add_argument('--arch', '-a', metavar='ARCH', default='version_2.4_CANFAN')
    ...
    # dataset
    parser.add_argument('--dataset', default='pancreas256',
                        help='dataset name')
    parser.add_argument('--img_ext', default='.npy',
                        help='image file extension')
    parser.add_argument('--mask_ext', default='.npy',
...
def main():
    ...
    arch = config['arch'].split('_', maxsplit=2)[-1]
    if config['name'] is None:
        if config['deep_supervision']:
            config['name'] = '%s_%s_wDS' % (config['dataset'], arch)
        else:
            config['name'] = '%s_%s_woDS' % (config['dataset'], arch)
    ...
    if isinstance(model_train, nn.Module):
        ...
        if arch == 'ClusterSeg':
            ...
            model = model_train(out_channels=1)
    model = model.cuda()
    ...
    # Data loading code
    print(os.path.join('inputs', config['dataset'], 'images', '*' + config['img_ext']))
